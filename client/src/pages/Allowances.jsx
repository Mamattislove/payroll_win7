...
export const loader = async ({ request }) => {
    try {
        ...
        if (allowanceType) params.set("allowanceType", allowanceType);

        const [{ data }, { data: atData }] = await Promise.all([
            customFetch.get(`/allowance-records?${params}`),
            customFetch.get("/allowance-types"),
        ]);
        return {
            allowanceRecords: data.allowanceRecords || [],
            totalAllowanceRecords: data.totalAllowanceRecords || 0,
            totalPages: data.totalPages || 1,
            currentPage: data.currentPage || 1,
            allowanceTypes: atData.allowanceTypes || [],
        };
    } catch (error) {
...
const RecordModal = ({ initial, allowanceTypes, onClose, onSaved }) => {
    const isEdit = !!initial;
    const [saving, setSaving] = useState(false);
    const [form, setForm] = useState({
        employee: initial?.employee?._id ?? initial?.employee ?? "",
        allowanceType: initial?.allowanceType?._id ?? initial?.allowanceType ?? "",
        name: initial?.name ?? "",
        amount: initial?.amount ?? "",
        allowanceDate: initial?.allowanceDate
            ? String(initial.allowanceDate).slice(0, 10)
            : "",
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const body = {
                ...
                allowanceDate: form.allowanceDate || null,
            };
            if (isEdit) {
                await customFetch.patch(`/allowance-records/${initial._id}`, body);
                toast.success("Allowance record updated");
            } else {
                body.employee = form.employee;
                await customFetch.post("/allowance-records", body);
                toast.success("Allowance record created");
            }
            onSaved();
        } catch (err) {
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                    <h2 className="font-semibold text-slate-800">
                        {isEdit ? "Edit Allowance Record" : "Add Allowance Record"}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <select value={form.allowanceType} onChange={set("allowanceType")} required className={inputCls}>
                            <option value="">— Select type —</option>
                            {allowanceTypes.map((t) => (
                                <option key={t._id} value={t._id}>{t.allowanceName}</option>
                            ))}
                        </select>
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            value={form.name}
                            onChange={set("name")}
                            placeholder="e.g. Transportation — Feb 2026"
                            required
                            className={inputCls}
                        />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        <label className={labelCls}>
                            Allowance Date (optional)
                        </label>
                        <input
                            type="date"
                            value={form.allowanceDate}
                            onChange={set("allowanceDate")}
                            className={inputCls}
                        />
                        <p className="text-[11px] text-slate-400">
                            Leave blank to put this on the next payroll run. Set
                            a date to hold it until a cutoff that reaches that
                            date — useful for keying one in early.
                        </p>
                    </div>
...
const Allowances = () => {
    ...
    const mayEdit = canWrite("allowanceRecords", user?.role);

    const revalidator = useRevalidator();
    const [searchParams, setSearchParams] = useSearchParams();

    const [filterEmployee, setFilterEmployee] = useState(searchParams.get("employee") || "");
    const [filterType, setFilterType] = useState(searchParams.get("allowanceType") || "");
    ...
    const handleDelete = (id) => {
        askConfirm("Delete this allowance record?", async () => {
            setDeleting(id);
            try {
                await customFetch.delete(`/allowance-records/${id}`);
        ...
        <div>
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Total: {totalAllowanceRecords}</p>
                </div>
                {mayEdit && (
                    <button
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4 flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:items-end">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <select
                        value={filterType}
                        onChange={(e) => { setFilterType(e.target.value); applyFilter("allowanceType", e.target.value); }}
            ...
            <div className="bg-white rounded-xl border border-slate-200">
                ...
                <div className="sm:hidden divide-y divide-slate-100">
                    {allowanceRecords.length === 0 && (
                        <p className="px-4 py-8 text-center text-slate-400">No allowance records found.</p>
                    )}
                        ...
                        <div key={rec._id} className="p-4">
                            <div className="flex items-start justify-between gap-2 mb-2">
                                <div className="min-w-0">
                                    ...
                                    <p className="text-xs text-slate-500 mt-0.5">{rec.allowanceType?.allowanceName ?? "—"} · {rec.name}</p>
                        ...
                        {allowanceRecords.map((rec, idx) => (
                            ...
                            >
                                ...
                                <td className="px-4 py-3 text-slate-600">
                                    {rec.allowanceType?.allowanceName ?? "—"}
                                </td>
