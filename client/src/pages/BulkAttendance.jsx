...
const leaveDayType = (leave) => {
    if (leave.halfday === LEAVE_HALFDAY.HALF_DAY)
        return DAY_TYPES.LEAVE_HALFDAY;
    if (leave.withPay === LEAVE_WITH_PAY.WITH_PAY)
        return DAY_TYPES.LEAVE_WITH_PAY;
...
const OverlaySection = ({ title, children }) => (
    <div>
        <h3 className="text-xs font-bold uppercase tracking-widest text-slate-500 border-b border-slate-100 pb-1 mb-3">
            {title}
        </h3>
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
                ...
                <div className="px-6 py-5 flex flex-col gap-7">
                    <OverlaySection title="Organization">
                        <OverlayField
                            label="Client"
                            value={designation?.client?.clientName}
                        />
                        <OverlayField
                            label="Department"
                            value={designation?.department?.departmentName}
                        />
                        <OverlayField
                            label="Position"
                            value={designation?.position?.positionName}
                        />
                    </OverlaySection>
                    ...
                    <OverlaySection title="Employment">
                        <OverlayField
                            ...
                            value={comp.employmentType}
                        />
                        <OverlayField
                            label="Contract Type"
                            value={comp.contractType}
                        />
                        <OverlayField
                            label="Payroll Period"
                            value={comp.payrollPeriod}
                        />
                        <OverlayField
                            label="Active Status"
                            value={comp.activeStatus}
                        />
                        <OverlayField
                            label="Start Contract"
                            value={fmtDate(comp.startContract)}
                        />
                        <OverlayField
                            label="End Contract"
                            value={fmtDate(comp.endContract)}
                        />
                        <OverlayField
                            label="Date of Regularization"
                            value={fmtDate(comp.dateOfRegularization)}
                        />
                    </OverlaySection>
                    ...
                    <OverlaySection title="Rates">
                        <OverlayField
                            label="Monthly Rate"
                            value={fmtMoney(comp.monthlyRate)}
                        />
                        <OverlayField
                            label="Daily Rate"
                            value={fmtMoney(comp.dailyRate)}
                        />
                    </OverlaySection>
                    ...
                    <OverlaySection title="Schedule">
                        <OverlayField
                            ...
                            value={formatDays(comp.weeklySchedule)}
                        />
                        <OverlayField
                            label="Rest Days"
                            value={formatRestDays(comp.weeklySchedule)}
                        />
                        <OverlayField label="Time In" value={comp.timeIn} />
                        <OverlayField label="Time Out" value={comp.timeOut} />
                        <OverlayField
                            label="Night Shift In"
                            value={comp.nightShiftTimeIn}
                        />
                        <OverlayField
                            label="Night Shift Out"
                            value={comp.nightShiftTimeOut}
                        />
                    </OverlaySection>
                    ...
                    <OverlaySection title="Government Contributions">
                        <OverlayField
                            label="SSS Basis"
                            value={comp.sssContributionBasis}
                        />
                        <OverlayField
                            label="SSS Overwrite"
                            value={
                                comp.sssOverwriteAmount != null
                                    ? fmtMoney(comp.sssOverwriteAmount)
                                    : "—"
                            }
                        />
                        <OverlayField
                            label="PhilHealth Basis"
                            value={comp.philhealthContributionBasis}
                        />
                        <OverlayField
                            label="PhilHealth Overwrite"
                            value={
                                comp.philhealthOverwriteAmount != null
                                    ? fmtMoney(comp.philhealthOverwriteAmount)
                                    : "—"
                            }
                        />
                        <OverlayField
                            label="Pag-IBIG Basis"
                            value={comp.pagibigContributionBasis}
                        />
                        <OverlayField
                            label="Pag-IBIG Overwrite"
                            value={
                                comp.pagibigOverwriteAmount != null
                                    ? fmtMoney(comp.pagibigOverwriteAmount)
                                    : "—"
                            }
                        />
                    </OverlaySection>
                    ...
                    {(comp.insuranceName ||
                        ...
                        <OverlaySection title="Insurance">
                            <OverlayField
                                ...
                                value={comp.insuranceName}
                            />
                            <OverlayField
                                label="Date Insured"
                                value={fmtDate(comp.dateInsured)}
                            />
                            <OverlayField
                                label="Insured Until"
                                value={fmtDate(comp.insuredUntil)}
                            />
                        </OverlaySection>
                    )}
                </div>
...
const SummaryModal = ({
    ...
    const totalHours =
        ...
        totals.nightPremiumHours +
        totals.overtimeNightPremiumHours;
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl">
                ...
                <div className="px-6 py-5 flex flex-col gap-5">
                                    ...
                                    {rows.map((row) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-3 py-1.5 text-right text-slate-600">
                                                {(
                                                    Number(
                                                        row.overtimeNightPremiumHours,
                                                    ) || 0
                                                ).toFixed(2)}
                                            </td>
                                            <td className="px-3 py-1.5 text-right text-slate-600">
                                                {(
                                                    Number(row.lateMin) || 0
                                                ).toFixed(0)}
                                            </td>
                                            <td className="px-3 py-1.5 text-right text-slate-600">
                                                {(
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        ...
                        />
                        <StatTile
                            label="Night Prem (hrs)"
                            value={totals.nightPremiumHours.toFixed(2)}
                        />
                        <StatTile
                            label="OT Night (hrs)"
                            value={totals.overtimeNightPremiumHours.toFixed(2)}
                        />
                        <StatTile
                            label="Late (mins)"
                        ...
                        <StatTile
                            label="Work Hours"
                            value={(totals.regularHours / 8).toFixed(2)}
                        />
                    </div>
...
const RowsEditor = ({
    ...
    const [rows, setRows] = useState(() =>
        generateDateRange(dateFrom, dateTo).map((d) =>
            blankRow(d, holidayMap, leaveMap),
        ),
    );
    ...
    const resetRow = (index) => {
        setRows((prev) =>
            prev.map((r, i) => {
                if (i !== index) return r;
                return {
                    ...blankRow(r.attendanceDate, holidayMap, leaveMap),
                    ...
                    {rows.map((row, idx) => (
                        <div key={row.attendanceDate} className="p-4">
                            ...
                            <div className="flex items-start justify-between gap-2 mb-3">
                                <div>
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        {row.existingId && (
                                            <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 leading-tight">
                                                Existing
                                            </span>
                                        )}
                                    </div>
                                    {holidayMap[row.attendanceDate] && (
                                        <span
                                            className={`inline-block text-[10px] font-medium mt-0.5 px-1.5 py-0.5 rounded leading-tight ${
                                                holidayMap[row.attendanceDate]
                                                    .specialLegal ===
                                                HOLIDAY_TYPES.LEGAL
                                    ...
                                    {leaveMap[row.attendanceDate] && (
                                        <span className="inline-block text-[10px] font-medium mt-0.5 px-1.5 py-0.5 rounded leading-tight bg-blue-100 text-blue-700">
                                            ...
                                            {leaveMap[row.attendanceDate]
                                                .halfday ===
                                            LEAVE_HALFDAY.HALF_DAY
                                                ? " (half)"
                                                : ""}
                                            {leaveMap[row.attendanceDate]
                                                .withPay ===
                                            LEAVE_WITH_PAY.WITH_PAY
                                                ? " · w/ pay"
                                                : ""}
                                        </span>
                                    )}
                            ...
                            <div className="flex items-center gap-2 mb-3">
                                {Object.entries(SHORTCUT_PANEL).map(
                                    ([
                                        key,
                                        { label, icon: Icon, badgeCls },
                                    ]) => (
                                        <button
                                            key={key}
                                            type="button"
                                            onClick={() =>
                                                applyShortcut(idx, key)
                                            }
                            ...
                            <div className="mb-3">
                                ...
                                {leaveMap[row.attendanceDate] ? (
                                    <div
                                        className="w-full border border-slate-200 rounded px-2 py-1.5 text-sm text-slate-400 bg-slate-50 cursor-not-allowed"
                            ...
                            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                                ...
                                <span className="font-semibold text-slate-800">
                                    ₱
                                    {rowTotalPay(row, dailyRate).toLocaleString(
                                        "en-PH",
                                        { minimumFractionDigits: 2 },
                                    )}
                                </span>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-slate-900 text-white text-xs uppercase tracking-wider">
                                ...
                                <th className="px-3 py-3 text-left min-w-45">
                                    Day Type
                                </th>
                            ...
                            {rows.map((row, idx) => (
                                ...
                                >
                                    <td className="px-3 py-2 whitespace-nowrap">
                                        ...
                                        {holidayMap[row.attendanceDate] && (
                                            <span
                                                className={`block text-[10px] font-medium mt-0.5 px-1.5 py-0.5 rounded leading-tight w-fit ${
                                                    holidayMap[
                                                        row.attendanceDate
                                                    ].specialLegal ===
                                                    HOLIDAY_TYPES.LEGAL
                        ...
                        <tfoot>
                            <tr className="bg-slate-800 text-white text-xs font-semibold border-t-2 border-slate-600">
                                <td
                                    colSpan={3}
                                    className="px-3 py-2 uppercase tracking-wider text-slate-300"
                                >
                                    Totals
                                ...
                                {[
                                    ...
                                    [
                                        "Total Pay",
                                        `₱${rows.reduce((s, r) => s + rowTotalPay(r, dailyRate), 0).toLocaleString("en-PH", { minimumFractionDigits: 2 })}`,
                                    ],
                                ].map(([label, value]) => (
                                    <td
                                        key={label}
                                    ...
                                    >
                                        <span className="block text-[9px] font-normal text-slate-400 mb-0.5">
                                            {label}
                                        </span>
                                        {value}
                                    </td>
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
                ...
                <p className="text-sm text-slate-600">
                    ...
                    <span className="font-semibold text-slate-900">
                        ...
                        {rows
                            .reduce(
                                (sum, row) => sum + rowTotalPay(row, dailyRate),
                                0,
                            )
                            .toLocaleString("en-PH", {
                                minimumFractionDigits: 2,
                ...
                >
                    {saving
                        ...
                        : (() => {
                              ...
                              const uc = rows.filter(
                                  (r) => r.existingId,
                              ).length;
                              if (nc > 0 && uc > 0)
                                  return `Submit ${nc} New + ${uc} Update(s)`;
...
const BulkAttendance = () => {
    ...
    useEffect(() => {
        ...
        customFetch
            .get(
                `/leave-applications?employee=${empId}&status=approved&limit=500`,
            )
            .then(({ data }) => {
                if (cancelled) return;
                const map = {};
                for (const leave of data.leaveApplications || []) {
                    let cur = new Date(
                        `${leave.dateFrom.slice(0, 10)}T00:00:00Z`,
                    );
                    const end = new Date(
                        `${leave.dateTo.slice(0, 10)}T00:00:00Z`,
                    );
                    while (cur <= end) {
    ...
    const handleDateFromChange = (e) => {
        ...
        if (day === 1) {
            setDateTo(
                new Date(Date.UTC(year, month, 15)).toISOString().slice(0, 10),
            );
        } else if (day === 16) {
    ...
    return (
        <>
            {showCompOverlay && selectedComp && (
                <CompensationOverlay
                    comp={selectedComp}
                    onClose={() => setShowCompOverlay(false)}
                />
            )}
            <div>
                <div className="mb-6">
                    ...
                    <p className="text-slate-500 mt-1">
                        Select an employee and a date range to generate one row
                        per date.
                    </p>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 mb-4 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-x-5 gap-y-3">
                    {Object.entries(SHORTCUT_PANEL).map(
                        ([shift, { label, icon: Icon, badgeCls }], i) => (
                            <div
                                key={shift}
                                className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2"
                            >
                                {i > 0 && (
                                    <span className="hidden sm:block w-px h-5 bg-slate-200 mr-3" />
                                )}
                                <div className="flex items-center gap-1.5">
                                    <span
                                        className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${badgeCls}`}
                                    >
                                        <Icon size={12} />
                                        {label}
                                    </span>
                                    {PRESET_QUICK_CONFIGS[shift]?.map(
                                        ({ label: qLabel, values }) => {
                                ...
                                <div className="grid grid-cols-4 gap-2 sm:contents">
                                    {PRESET_FIELDS.map(
                                        ({ key, label: fieldLabel }) => (
                                            <label
                                                key={key}
                                                className="flex flex-col sm:flex-row sm:items-center gap-1 text-xs text-slate-500"
                                            >
                ...
                {hasRange ? (
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-400 text-sm">
                        Select both From and To dates to generate attendance
                        rows.
                    </div>
                )}
            </div>
