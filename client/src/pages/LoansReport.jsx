...
// One line per loan payment rather than per employee: someone paying an SSS
// salary loan and a Pag-IBIG MP loan in the same cutoff has two deductions to
// account for, and collapsing them hides which loan was collected.
    ...
    for (const p of payrolls) {
        ...
        for (const lp of p.loans || []) {
            ...
            rows.push({
                ...
                // Legacy imported loans carry no loanName, so the type is the
                // only label they have.
                loan:
                    lp.loan?.loanName ||
                ...
                // What is left after this cutoff's payment: the payroll run
                // decrements loanPayable as it creates the payment.
                balance: lp.loan?.loanPayable ?? null,
            });
        }
    }
    rows.sort(
...
const W = { num: 24, code: 56, loan: 120, period: 74, amount: 76, bal: 76 };
        ...
        <Document>
            <Page size="A4" style={S.page}>
                <View style={S.header}>
                    ...
                    <Text style={S.rptTitle}>LOANS REPORT</Text>
                ...
                <View style={S.row} fixed>
                    ...
                    <View style={[S.th, { width: W.loan }]}>
                        <Text style={S.thText}>Loan</Text>
                    </View>
                    {showPeriod && (
                    ...
                    <View style={[S.th, { width: W.bal }]}>
                        <Text style={S.thText}>Balance</Text>
                    </View>
                </View>

                {rows.length === 0 ? (
                    <Text style={S.empty}>
                        No loan deductions for this client and period.
                    </Text>
                ) : (
                    rows.map((row, i) => {
                        ...
                        return (
                            <View key={i} style={S.row} wrap={false}>
                                ...
                                <View style={[S.td, bg, { width: W.loan }]}>
                                    <Text style={S.tdL}>{row.loan}</Text>
                                </View>
                                {showPeriod && (
                                    <View
                                ...
                                <View style={[S.td, bg, { width: W.bal }]}>
                                    <Text style={S.tdR}>
                                        {row.balance == null
                                            ? "—"
                                            : f2(row.balance)}
                ...
                {rows.length > 0 && (
                    <View style={[S.row, S.gt]}>
                        ...
                        <View style={[S.td, S.gt, { width: W.loan }]}>
                            <Text style={S.gtLabel}> </Text>
                        </View>
                        {showPeriod && (
                        ...
                        <View style={[S.td, S.gt, { width: W.bal }]}>
                            <Text style={S.gtLabel}> </Text>
                        </View>
                    </View>
                )}
...
const LoansReport = () => {
    ...
    const downloadPDF = async () => {
        ...
        setPdfLoading(true);
        try {
            const blob = await pdf(<LoansPDF report={report} />).toBlob();
            const url = URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `loans-report-${report.dateFrom}-to-${report.dateTo}.pdf`;
    ...
    const total = report ? sumRows(report.rows) : 0;
    const colSpan = report?.showPeriod ? 6 : 5;
            ...
            {report && (
                <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                    <div className="text-center mb-4 pb-4 border-b border-slate-200">
                        ...
                        <p className="font-bold text-sm mt-2">LOANS REPORT</p>
                        ...
                        <table className="border-collapse min-w-full">
                            <thead>
                                <tr>
                                    ...
                                    <Th>Employee Name</Th>
                                    <Th>Loan</Th>
                                    {report.showPeriod && <Th>Period</Th>}
                            ...
                            <tbody>
                                {report.rows.length === 0 ? (
                                    <tr>
                                        ...
                                        >
                                            No loan deductions for this client
                                            and period.
                                        </td>
                                    </tr>
                                ) : (
