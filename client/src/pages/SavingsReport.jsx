...
// "Aug. 16, 2026" — the form the printed report uses, which en-PH's short
...
// Compact "Aug 16 – Aug 31" for the per-row pay period.
...
// One line per employee per pay period, carrying that period's savings
// deduction. The deduction is the sum of the payroll's linked savings records
// because an employee can hold more than one plan, and the payroll totals add
// them the same way.
function buildRows(payrolls) {
    const rows = payrolls
        .map((p) => {
            ...
            return {
                ...
                deduction: (p.savings || []).reduce(
                    (s, r) => s + (r.amount || 0),
                    0,
                ),
            };
        })
        // This is a report of who was deducted. Everyone on the payroll without
        // a savings plan would otherwise pad it with zero-peso lines.
    ...
    // A single cutoff prints exactly like the paper report. A wider range
    // returns several periods per employee, and without the period column those
    // land as indistinguishable repeated names.
...
const W = { num: 26, code: 62, period: 78, amount: 96 };
        ...
        <Document>
            <Page size="A4" style={S.page}>
                <View style={S.header}>
                    ...
                    <Text style={S.rptTitle}>SAVINGS REPORT</Text>
                ...
                {rows.length === 0 ? (
                    <Text style={S.empty}>
                        No savings deductions for this client and period.
                    </Text>
                ) : (
                    rows.map((row, i) => {
...
const SavingsReport = () => {
    ...
    const downloadPDF = async () => {
        ...
        setPdfLoading(true);
        try {
            const blob = await pdf(<SavingsPDF report={report} />).toBlob();
            const url = URL.createObjectURL(blob);
            const a = document.createElement("a");
            a.href = url;
            a.download = `savings-report-${report.dateFrom}-to-${report.dateTo}.pdf`;
            ...
            {report && (
                <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                    <div className="text-center mb-4 pb-4 border-b border-slate-200">
                        ...
                        <p className="font-bold text-sm mt-2">SAVINGS REPORT</p>
                        ...
                        <table className="border-collapse min-w-full">
                            ...
                            <tbody>
                                {report.rows.length === 0 ? (
                                    <tr>
                                        <td
                                            colSpan={report.showPeriod ? 5 : 4}
                                            className="text-center py-10 text-slate-400 text-sm"
                                        >
                                            No savings deductions for this
                                            client and period.
                                        </td>
                                    </tr>
                                ) : (
