...
export const loader = async ({ params }) => {
    try {
        const [{ data }, { data: desigData }] = await Promise.all([
            customFetch.get(`/employees/${params.id}`),
            customFetch.get("/employee-designations", {
                params: { employee: params.id, limit: 50 },
            }),
        ]);

        const designations = await Promise.all(
            (desigData.employeeDesignations || []).map(async (d) => {
        ...
        return { employee: data.employee, designations };
    } catch (error) {
...
// Converts a MongoDB ISO date string to the "YYYY-MM-DD" format date inputs need
const toDate = (val) => (val ? val.slice(0, 10) : "");

const fmtDate = (val) =>
    val ? new Date(val).toLocaleDateString("en-PH", { year: "numeric", month: "short", day: "numeric" }) : "—";
...
const EditEmployee = () => {
    const { employee, designations } = useLoaderData();
    const navigate = useNavigate();
    const navigation = useNavigation();
    const submitting = navigation.state === "submitting";

    const [families, setFamilies] = useState(() =>
        employee.families?.length
    ...
    const [workExp, setWorkExp] = useState(() =>
        employee.workingExperience?.length
            ? employee.workingExperience.map((w) => ({
                  company: w.company || "",
                  position: w.position || "",
                  from: toDate(w.from),
                  to: toDate(w.to),
    ...
    return (
        <div>
            ...
            {/* Designation & Compensation (read-only, if available) */}
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden mb-6">
                <SectionHeader>Designation &amp; Compensation</SectionHeader>
                <div className="p-4">
                    {designations.length === 0 ? (
                        <p className="text-sm text-slate-400">No designation on record.</p>
                    ) : (
                        <div className="flex flex-col gap-4">
                            {designations.map((d) => (
                                <div key={d._id} className="rounded-lg border border-slate-100 bg-slate-50 p-3">
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3">
                                        <InfoField label="Client" value={d.client?.clientName} />
                                        <InfoField label="Department" value={d.department?.departmentName} />
                                        <InfoField label="Position" value={d.position?.positionName} />
                                    </div>
                                    {d.compensation ? (
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 mt-3 pt-3 border-t border-slate-200">
                                            <InfoField label="Monthly Rate" value={fmtMoney(d.compensation.monthlyRate)} />
                                            <InfoField label="Daily Rate" value={fmtMoney(d.compensation.dailyRate)} />
                                            <InfoField label="Contract Type" value={d.compensation.contractType} capitalize />
                                            <InfoField label="Employment Type" value={d.compensation.employmentType} capitalize />
                                            <InfoField label="Payroll Period" value={d.compensation.payrollPeriod} capitalize />
                                            <InfoField label="Active Status" value={d.compensation.activeStatus} capitalize />
                                            <InfoField label="Start Contract" value={fmtDate(d.compensation.startContract)} />
                                            <InfoField label="End Contract" value={fmtDate(d.compensation.endContract)} />
                                            <InfoField label="Date of Regularization" value={fmtDate(d.compensation.dateOfRegularization)} />
                                        </div>
                                    ) : (
                            ...
                            <div className="p-4 flex flex-col gap-3">
                                ...
                                <Field label="Other Address">
                                    <textarea
                                        name="otherAddress"
                                        rows={2}
                                        defaultValue={employee.otherAddress}
                                        className={inputCls}
                                        placeholder="No. Street Barangay Municipality/City Province"
                                    />
                                </Field>
                            ...
                            <div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <Field label="PhilHealth">
                                    <input
                                        ...
                                        defaultValue={employee.philhealthNumber}
                                        className={inputCls}
                                    />
                                </Field>
                                <Field label="TIN">
                                    <input
                                        name="tinNumber"
                                        defaultValue={employee.tinNumber}
                                        className={inputCls}
                                    />
                                </Field>
                                <Field label="GSIS">
                                    <input
                                        name="gsisNumber"
                                        defaultValue={employee.gsisNumber}
                                        className={inputCls}
                                    />
                                </Field>
                            </div>
                            ...
                            <div className="p-4 flex flex-col gap-3">
                                ...
                                <Field label="Vocational Course">
                                    <input
                                        ...
                                        defaultValue={
                                            employee.profession?.vocational
                                        }
                                        className={inputCls}
                                    />
                                </Field>
