...
function buildRows(payrolls) {
    return payrolls
        .map((p) => {
            ...
            const leaveSil =
                (p.earning || []).reduce((s, e) => s + (e.amount || 0), 0) +
                (p.leavePay || 0);
            ...
            const sssLoan = (p.loans || [])
                .filter((l) =>
                    /sss/i.test(l.loan?.loanType?.loanTypeName || ""),
                )
                .reduce((s, l) => s + (l.amount || 0), 0);
            const hdmfLoan = (p.loans || [])
                .filter((l) =>
                    /hdmf|pag.?ibig/i.test(
                        l.loan?.loanType?.loanTypeName || "",
                    ),
                )
                .reduce((s, l) => s + (l.amount || 0), 0);
            const tax = p.withholdingTax || 0;
            const allowances = (p.allowances || []).reduce(
                (s, a) => s + (a.amount || 0),
                0,
            );
            const netPay = p.netSalary || p.finalPay || 0;
            // The catch-all column is whatever reconciles gross to the net that
            // was actually paid, rather than totalDeductions minus the itemised
            // columns. Those two agree for a payroll this app generated, but
            // 25 of 635 imported rows in a recent cutoff carry a stored net
            // that is not gross minus totalDeductions, and deriving from
            // totalDeductions left those rows not adding across the page.
            // Gross and net stay exactly as paid; the discrepancy lands here,
            // where it is visible and labelled, instead of breaking the row.
            const deductions = r2(
                gross - netPay - sss - ph - hdmf - tax - sssLoan - hdmfLoan,
            );
            ...
            <Page size={[936, 612]} style={S.page}>
                <View style={S.header}>
                    ...
                    <Text style={S.rptDate}>
                        Date Covered: {fmtDate(dateFrom)} – {fmtDate(dateTo)}
                    </Text>
                </View>
                <View style={S.meta}>
                    <Text>
                ...
                <View style={S.row}>
                    {[
                        ...
                        <View
                            key={label}
                            style={[S.th, flex ? { flex: 1 } : { width: w }]}
                        >
                ...
                {rows.map((row, i) => {
                    ...
                    return (
                        <View key={i} style={S.row}>
                            ...
                            <View style={[S.td, bg, { width: W.rate }]}>
                                <Text style={S.tdR}>{f2(row.dailyRate)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.basic }]}>
                                <Text style={S.tdR}>{f2(row.basicPay)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.days }]}>
                                <Text style={S.tdR}>{f4(row.days)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.ot }]}>
                                <Text style={S.tdR}>{f2(row.otHolSun)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.nd }]}>
                                <Text style={S.tdR}>{f2(row.nd)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.leave }]}>
                                <Text style={S.tdR}>{f2(row.leaveSil)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.allw }]}>
                                <Text style={S.tdR}>{f2(row.allowances)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.gross }]}>
                                <Text style={S.tdR}>{f2(row.gross)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.sss }]}>
                                <Text style={S.tdR}>{f2(row.sss)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.ph }]}>
                                <Text style={S.tdR}>{f2(row.ph)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.hdmf }]}>
                                <Text style={S.tdR}>{f2(row.hdmf)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.sssLoan }]}>
                                <Text style={S.tdR}>{f2(row.sssLoan)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.hdmfLoan }]}>
                                <Text style={S.tdR}>{f2(row.hdmfLoan)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.tax }]}>
                                <Text style={S.tdR}>{f2(row.tax)}</Text>
                            </View>
                            <View style={[S.td, bg, { width: W.ded }]}>
                                <Text style={S.tdR}>{f2(row.deductions)}</Text>
                            </View>
                ...
                <View style={[S.row, S.gt]}>
                    ...
                    <View style={[S.td, S.gt, { width: W.rate }]}>
                        <Text style={S.gtLabel}> </Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.basic }]}>
                        <Text style={S.gtAmt}>{f2(t.basicPay)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.days }]}>
                        <Text style={S.gtAmt}>{f4(t.days)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.ot }]}>
                        <Text style={S.gtAmt}>{f2(t.otHolSun)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.nd }]}>
                        <Text style={S.gtAmt}>{f2(t.nd)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.leave }]}>
                        <Text style={S.gtAmt}>{f2(t.leaveSil)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.allw }]}>
                        <Text style={S.gtAmt}>{f2(t.allowances)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.gross }]}>
                        <Text style={S.gtAmt}>{f2(t.gross)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.sss }]}>
                        <Text style={S.gtAmt}>{f2(t.sss)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.ph }]}>
                        <Text style={S.gtAmt}>{f2(t.ph)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.hdmf }]}>
                        <Text style={S.gtAmt}>{f2(t.hdmf)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.sssLoan }]}>
                        <Text style={S.gtAmt}>{f2(t.sssLoan)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.hdmfLoan }]}>
                        <Text style={S.gtAmt}>{f2(t.hdmfLoan)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.tax }]}>
                        <Text style={S.gtAmt}>{f2(t.tax)}</Text>
                    </View>
                    <View style={[S.td, S.gt, { width: W.ded }]}>
                        <Text style={S.gtAmt}>{f2(t.deductions)}</Text>
                    </View>
    ...
    return (
        <tr className={bg}>
            ...
            <td className={tdc}>
                {row.code}
                {row.imported && (
                    <span title="Imported from the previous system">*</span>
                )}
            </td>
