...
const RecordModal = ({ initial, earningTypes, onClose, onSaved }) => {
    ...
    const [form, setForm] = useState({
        ...
        earningDate: initial?.earningDate
            ? String(initial.earningDate).slice(0, 10)
            : "",
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const body = {
                ...
                earningDate: form.earningDate || null,
            };
            if (isEdit) {
                ...
                <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                        <label className={labelCls}>
                            Earning Date (optional)
                        </label>
                        <input
                            type="date"
                            value={form.earningDate}
                            onChange={set("earningDate")}
                            className={inputCls}
                        />
...
const Earnings = () => {
    ...
    const mayEdit = canWrite("earningRecords", user?.role);
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-xs text-slate-400 mt-0.5">
                                        {rec.payroll
                                            ...
                                            : rec.earningDate
                                              ? `Standing — for ${fmtDate(rec.earningDate)}`
                                              : "Standing (next run)"}
                                    </p>
                                </div>
                                {mayEdit && (
