...
export const loader = async ({ request }) => {
    try {
        ...
        const [{ data }, { data: ctData }] = await Promise.all([
            customFetch.get(`/charge-records?${params}`),
            customFetch.get("/charge-types"),
        ]);
        return {
            chargeRecords: data.chargeRecords || [],
            totalChargeRecords: data.totalChargeRecords || 0,
            totalPages: data.totalPages || 1,
            currentPage: data.currentPage || 1,
            chargeTypes: ctData.chargeTypes || [],
        };
    } catch (error) {
...
const RecordModal = ({ initial, chargeTypes, onClose, onSaved }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                ...
                toast.success("Charge record updated");
            } else {
                body.employee = form.employee;
                await customFetch.post("/charge-records", body);
                toast.success("Charge record created");
            }
            onSaved();
        } catch (err) {
                ...
                <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            value={form.name}
                            onChange={set("name")}
                            placeholder="e.g. Savings — Feb 2026"
                            required
                            className={inputCls}
                        />
                    </div>
...
const Charges = () => {
    ...
    const handleDelete = (id) => {
        askConfirm("Delete this charge record?", async () => {
            setDeleting(id);
            try {
                await customFetch.delete(`/charge-records/${id}`);
